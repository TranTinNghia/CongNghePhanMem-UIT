...
class CustomerSearchService:
    ...
    def get_monthly_revenue(self, tax_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            cursor.execute(
                """SELECT YEAR(r.receipt_date) as year, MONTH(r.receipt_date) as month,
                          SUM(l.amount) as total_amount, COUNT(DISTINCT l.container_number) as container_count
                   FROM dbo.receipts r
                   INNER JOIN dbo.customers c ON r.customer_key = c.customer_key
                   INNER JOIN dbo.lines l ON r.receipt_key = l.receipt_key
                   WHERE c.tax_code = ? AND c.is_active = N'Y'
