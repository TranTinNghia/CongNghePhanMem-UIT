...
class ServiceExtractor:
    ...
    def extract_service_info(self, option: str, unit_price: int, receipt_date: str, tax_rate: Optional[int] = None) -> Optional[Dict]:
        if not option or not receipt_date:
            print(f"[ServiceExtractor] Skipping: option={option}, receipt_date={receipt_date}")
            return None
        
        option_lower = option.lower()
        print(f"[ServiceExtractor] Processing option: '{option}'")
        ...
        if "phụ thu phí nâng" in option_lower:
            ...
            if arrow_pattern:
                ...
                print(f"[ServiceExtractor] Found arrow pattern ->: {service_name}")
            elif lon_hon_pattern:
                service_name = f"Phụ thu phí nâng lớn hơn {lon_hon_pattern.group(1)} ngày"
                print(f"[ServiceExtractor] Found 'lớn hơn' pattern: {service_name}")
            elif so_ngay_pattern:
                service_name = f"Phụ thu phí nâng {so_ngay_pattern.group(1)} ngày"
                print(f"[ServiceExtractor] Found day number pattern: {service_name}")
            else:
        ...
        if not service_name or not container_size or not container_status:
            print(f"[ServiceExtractor] Missing information: service_name={service_name}, container_size={container_size}, container_status={container_status}")
            return None
        ...
        if not from_date or not to_date:
            print(f"[ServiceExtractor] Could not parse date: from_date={from_date}, to_date={to_date}, receipt_date={receipt_date}")
            return None
        ...
        print(f"[ServiceExtractor] Extract successful: {result}")
        return result
