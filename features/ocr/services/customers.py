    ...
    def save_customer_scd2(self, tax_code: str, customer_name: str, address: str, province_key: Optional[str]) -> bool:
        ...
        try:
            ...
            if existing_customer:
                ...
                cursor.execute(
                    "INSERT INTO dbo.customers (tax_code, customer_name, address, province_key, start_time) VALUES (?, ?, ?, ?, ?)",
                    (tax_code, customer_name, address, province_key, current_timestamp)
                )
            else:
                cursor.execute(
                    "INSERT INTO dbo.customers (tax_code, customer_name, address, province_key) VALUES (?, ?, ?, ?)",
