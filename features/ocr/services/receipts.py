        ...
        try:
            ...
            if existing_receipt:
                ...
                if not has_changes:
                    ...
                    return True
                cursor.execute(
                    "UPDATE dbo.receipts SET receipt_code = ?, receipt_date = ?, shipment_code = ?, invoice_number = ?, customer_key = ? WHERE receipt_key = ?",
                    (receipt_code, converted_date, shipment_code, invoice_number, customer_key, old_receipt_key)
                )
            else:
                cursor.execute(
                    "INSERT INTO dbo.receipts (receipt_code, receipt_date, shipment_code, invoice_number, customer_key) VALUES (?, ?, ?, ?, ?)",
