    ...
    def save_container_scd2(self, container_size: str, container_status: str, container_type: str) -> bool:
        ...
        try:
            ...
            if existing_container:
                ...
                cursor.execute(
                    "INSERT INTO dbo.containers (container_size, container_status, container_type, start_time) VALUES (?, ?, ?, ?)",
                    (container_size, container_status, container_type, current_timestamp)
                )
            else:
                cursor.execute(
                    "INSERT INTO dbo.containers (container_size, container_status, container_type) VALUES (?, ?, ?)",
                    (container_size, container_status, container_type)
                )
            conn.commit()
            conn.close()
