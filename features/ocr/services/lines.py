        ...
        try:
            ...
            if existing_line:
                ...
                if not has_changes:
                    ...
                    return True
                cursor.execute(
                    "UPDATE dbo.lines SET receipt_key = ?, container_number = ?, service_key = ?, quantity = ?, discount = ?, amount = ? WHERE line_key = ?",
                    (receipt_key, container_number, service_key, quantity, discount, amount, old_line_key)
                )
            else:
                cursor.execute(
                    "INSERT INTO dbo.lines (receipt_key, container_number, service_key, quantity, discount, amount) VALUES (?, ?, ?, ?, ?, ?)",
